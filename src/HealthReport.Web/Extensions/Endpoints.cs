...
public static class Endpoints
{
    public static WebApplication MapEndpoint(this WebApplication app)
    {
        ...
        app.MapGet("/api/measurements/glucose/xlsx", async (IBloodGlucoseMeasurementsHandler handler, DateOnly from, DateOnly to, ClaimsPrincipal user, CancellationToken ct) =>
            {
                if (user?.Identity is null || !user.Identity.IsAuthenticated)
                    return Results.Unauthorized();
                ...
                return Results.File(BloodGlucoseMeasurementsToXlsxHandler.Export(data),
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", $"glucose_{from}-{to}.xlsx");
            })
            .WithTags("Measurements")
            .Produces<byte[]>(StatusCodes.Status200OK ,"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            .Produces<ApiError>(StatusCodes.Status400BadRequest, "application/json")
